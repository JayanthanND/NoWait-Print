        ...
        <div className="sticky bottom-0 border-t border-border bg-card p-4">
          <div className="flex gap-3">
            {order.status === "pending" && (
              <Button className="flex-1" onClick={() => handleStatusUpdate("processing")} disabled={loading}>
                {loading && <Spinner className="mr-2 h-4 w-4 animate-spin" />}
                Start Processing
              </Button>
            )}
            {order.status === "processing" && (
              <Button className="flex-1" onClick={() => handleStatusUpdate("printing")} disabled={loading}>
                {loading && <Spinner className="mr-2 h-4 w-4 animate-spin" />}
                Start Printing
              </Button>
            )}
            {order.status === "printing" && (
              <Button className="flex-1" onClick={() => handleStatusUpdate("ready")} disabled={loading}>
                {loading && <Spinner className="mr-2 h-4 w-4 animate-spin" />}
                Mark Ready
              </Button>
            )}
            {order.status === "ready" && (
              <Button className="flex-1" onClick={() => handleStatusUpdate("completed")} disabled={loading}>
                {loading && <Spinner className="mr-2 h-4 w-4 animate-spin" />}
                Complete Order
              </Button>
            )}
            <Button variant="outline" disabled={loading}>Print Receipt</Button>
          </div>
        </div>
      </SheetContent>
    </Sheet>
