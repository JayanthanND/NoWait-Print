...
import { Work, WorkProperties, PAPER_SIZES, PAPER_TYPES, GSM_OPTIONS, BINDING_OPTIONS, FileItem } from "../../types";
...
export function WorkPropertiesModal({ work, isOpen, onClose, onSave }: WorkPropertiesModalProps) {
  ...
  const currentPrice = calculateWorkPrice(currentWork);

  // Calculate prices for each option
  const paperSizeOptions = PAPER_SIZES.map((size) => ({
    value: size.value,
    label: size.label,
    price: calculateOptionPrice(currentWork, "paperSize", size.value),
  }));

  const paperTypeOptions = PAPER_TYPES.map((type) => ({
    value: type.value,
    label: type.label,
    price: calculateOptionPrice(currentWork, "paperType", type.value),
  }));

  const gsmOptions = GSM_OPTIONS.map((gsm) => ({
    value: gsm.value,
    label: gsm.label,
    price: calculateOptionPrice(currentWork, "gsm", gsm.value),
  }));

  const bindingOptions = BINDING_OPTIONS.map((binding) => ({
    value: binding.value,
    label: binding.label,
    price: calculateOptionPrice(currentWork, "binding", binding.value),
  }));
  ...
  const sidedOptions = [
    {
      value: "single",
      label: "Single",
      price: calculateOptionPrice(currentWork, "sided", "single"),
    },
    {
      value: "double",
      label: "Double",
      price: calculateOptionPrice(currentWork, "sided", "double"),
    },
  ];
  ...
  const colorOffPrice = calculateOptionPrice(currentWork, "color", false);
  const colorOnPrice = calculateOptionPrice(currentWork, "color", true);
